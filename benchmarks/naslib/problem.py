...
import naslib.search_spaces as ss
from kurobako import problem
import naslib.search_spaces.transbench101.graph as transbench101_graph
import naslib.search_spaces.nasbench201.graph as nasbench201_graph
...
class NASLibProblemFactory(problem.ProblemFactory):
    ...
    def specification(self):
        if isinstance(self._search_space, ss.TransBench101SearchSpaceMicro):
            params = [problem.Var(f"x{i}", problem.CategoricalRange(transbench101_graph.OP_NAMES)) for i in range(6)]
        ...
        #     self._converter = lambda x: [z * 8 for z in x]
            ...
            params = [problem.Var(f"x{i}", problem.CategoricalRange(nasbench201_graph.OP_NAMES)) for i in range(6)]
            self._converter = lambda x: [int(z) for z in x]
        else:
...
class NASLibProblem(problem.Problem):
    def __init__(self, search_space: Graph, converter, direction: str, **config: Dict[str, Any]) -> None:
        """
        params:
    ...
    def create_evaluator(self, params):
        return NASLibEvaluator(self._converter(params), self._search_space.copy(), self._scale, self._config)


class NASLibEvaluator(problem.Evaluator):
    def __init__(self, params: List[Optional[float]], search_space: Graph, scale: float, config: Dict[str, Any]) -> None:
        self._search_space = search_space
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 1 + 2:
        ...
        print("Example: python3 nas_bench_suite/problems.py nasbench201 cifar10 \"{'metric':'TEST_ACCURACY'}\"")
        exit(1)
    ...
    runner = problem.ProblemRunner(NASLibProblemFactory(**config))
    runner.run()
