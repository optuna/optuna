...
class _ParzenEstimator:
    ...
    def sample(self, rng: np.random.RandomState, size: int) -> Dict[str, np.ndarray]:
        ...
        samples_dict = {param_name: np.array([]) for param_name in self._search_space.keys()}


        def sample_one(rng: np.random.RandomState) -> Dict[str, float]:
            sample = {}
            active = rng.choice(len(self._weights), p=self._weights)
            ...
            for param_name, dist in self._search_space.items():
                ...
                if isinstance(dist, distributions.CategoricalDistribution):
                    ...
                    sample[param_name] = rng.choice(len(weights), p=weights)
        ...
        samples = [sample_one(rng) for _ in range(size)]
        samples_dict = {param_name: np.array([samples[i][param_name] for i in range(size)]) for param_name in self._search_space.keys()}
