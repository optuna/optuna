...
import torch
import math
from ._gp import KernelParams, kernel, MATERN_KERNEL0, posterior
from ._search_space import SearchSpace, CATEGORICAL
import numpy as np
...
def eval_acqf_no_grad(acqf: Acqf, x: np.ndarray) -> float:
    with torch.no_grad():
...
def eval_acqf_with_grad(acqf: Acqf, x: np.ndarray) -> tuple[float, np.ndarray]:
    ...
    val = eval_acqf(acqf, x_torch)
    val.backward()
    return val.detach().numpy(), x_torch.grad.numpy()
