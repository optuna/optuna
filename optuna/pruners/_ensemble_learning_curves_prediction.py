...
class EnsembleLearningCurvesPruner(BasePruner):
    ...
    def __init__(self, max_epoch, maximize=False, yrange=None, n_startup_trials=5, n_warmup_steps=0, interval_steps=1, n_estimators=100,
        ...
        self._maximum_possible_std = None if math.isnan(yrange[1] - yrange[0]) or maximum_possible_std is None \
    ...
    def gradient_descent(self, f_curr, f_prev):
        """
        Parameters
        ----------
    ...
    def _implement_gradient_descent(self, f_curr_org, f_prev_org, n_startpoints=100):
        ...
        for i in range(int(2 * n_startpoints)):
            a, b, L = self.gradient_descent(f_curr, f_prev)
            if not self.monotonicity_condition or a * f_prev[-1] + b >= f_curr_max:
    ...
    def fit(self):
        ...
        for i, y_prev in enumerate(self.ys_prev):
            ...
            for ai, bi, li in zip(a, b, loss):
                A.append(a), B.append(b), Ls.append(loss), id.append(i)

        A, B, Ls, id = map(np.asarray, (A, B, Ls, id))
        order = np.argsort(Ls)[::-1][:self._n_estimators] if self._maximize else np.argsort(Ls)[:self._n_estimators]
    ...
    def prune(self, study, trial):
        ...
        y_best, y_curr_best = self.y_prev.max(), self.y_curr.max()
        ...
        p_better = self._posterior_probability_better_than(y_best, self.epoch, mean, std)
        print('P(y is better than y_best) = {}\n'.format(p_better))

        if p_better < self.threshold:
