...
class RDBStorage(BaseStorage):
    ...
    def set_study_direction(
        ...
        study = models.StudyModel.find_or_raise_by_id(study_id, session)
        prev_direction = tuple([d.direction for d in study.direction])

        if prev_direction[0] != StudyDirection.NOT_SET and prev_direction != direction:
            raise ValueError(
                "Cannot overwrite study direction from {} to {}.".format(prev_direction, direction)
            )

        study.direction = [models.StudyDirectionModel(direction=d) for d in direction]
    ...
    def get_study_direction(
        ...
        return tuple([d.direction for d in direction])
    ...
    def get_all_study_summaries(self) -> List[StudySummary]:
        ...
        study_summary_stmt = session.query(
            ...
            orm.outerjoin(
                orm.join(models.StudyModel, models.StudyDirectionModel), summarized_trial
            )
        )
    ...
    def _update_trial(
        ...
        if value is not None:
            if not isinstance(value, Sequence):
                value = [value]
            trial_model.value = [models.TrialValueModel(value=v) for v in value]
        ...
        if intermediate_values:
            ...
            for value_model in value_models:
                if value_model.step in value_dict:
                    ...
                    value_dict[value_model.step] = [value_model]
            for s, v in intermediate_values.items():
                if not isinstance(v, Sequence):
                    v = [v]
                if s in value_dict:
                    for i, x in enumerate(v):
                        value_dict[s][i].value = x
                        session.add(value_dict[s][i])
                else:
                    trial_model.intermediate_values.extend(
                        [models.TrialIntermediateValueModel(step=s, value=x) for x in v]
                    )
    ...
    def set_trial_value(self, trial_id: int, value: Union[float, Sequence[float]]) -> None:
        ...
        if not isinstance(value, Sequence):
            value = [value]
        trial.value = [models.TrialValueModel(value=v) for v in value]
    ...
    def _set_trial_intermediate_value_without_commit(
        ...
        if not isinstance(intermediate_value, Sequence):
            intermediate_value = [intermediate_value]

        trial_values = models.TrialIntermediateValueModel.find_by_trial_and_step(
            trial, step, session
        )
        if trial_values is None:
            for v in intermediate_value:
                trial_value = models.TrialIntermediateValueModel(
                    trial_id=trial_id, step=step, value=v
                )
                session.add(trial_value)
        else:
            for i, v in enumerate(intermediate_value):
                trial_values[i].value = v
    ...
    def _get_trials(self, study_id: int, excluded_trial_ids: Set[int]) -> List[FrozenTrial]:
        ...
        try:
            trial_models = (
                ...
                .options(orm.selectinload(models.TrialModel.value))
    ...
    # TODO(hvy): Handle multi-objective.
