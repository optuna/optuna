...
class RDBStorage(BaseStorage, BaseHeartbeat):
    ...
    class _CachedStorage(BaseStorage, BaseHeartbeat):
        ...
        :class:`~optuna.storages._CachedStorage` meets following **Data persistence** requirements.

        **Data persistence**

        :class:`~optuna.storages._CachedStorage` do not guarantee that write operations are logged
            ...
            with self._lock:
                if study_id in self._studies:
                    for trial_number in self._studies[study_id].trials:
                        trial_id = self._study_id_and_number_to_trial_id.get(
        ...
        def get_all_trials(
            ...
            with self._lock:
                study = self._studies[study_id]
                # We need to sort trials by their number as some samplers assume this behavior.
