...
def test_float_without_nan_to_stored_repr(value: float) -> None:
    assert (
        RDBStorage._stored_repr_to_float_without_nan(
            *RDBStorage._float_without_nan_to_stored_repr(value)
        )
        == value
    )
...
def test_float_with_nan_to_stored_repr(value: float) -> None:
    def nanequal(x: float, y: float) -> bool:
        return (np.isnan(x) and np.isnan(y)) or x == y

    assert nanequal(
        RDBStorage._stored_repr_to_float_with_nan(
            *RDBStorage._float_with_nan_to_stored_repr(value)
        ),
        value,
    )
