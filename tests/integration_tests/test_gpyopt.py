...
def test_conversion_from_distribution_to_dimension() -> None:
    ...
    with patch("GPyOpt.methods.BayesianOptimization") as mock_object:
        study.optimize(_objective, n_trials=2, catch=())

        domain = mock_object.mock_calls[0][2]["domain"]
        expected_domain = [
            # Original: trial.suggest_uniform('p0', -3.3, 5.2)
            {"name": "p0", "type": "continuous", "domain": (-3.3, 5.2)},
            # Original: trial.suggest_uniform('p1', 2.0, 2.0)
            {"name": "p1", "type": "continuous", "domain": (2.0, 2.0)},
            # Original: trial.suggest_categorical('p10', ['9', '3', '0', '8'])
            {"name": "p10", "type": "categorical", "domain": ["9", "3", "0", "8"]},
            # Original: trial.suggest_loguniform('p2', 0.0001, 0.3)
            {"name": "p2", "type": "continuous", "domain": (np.log(0.0001), np.log(0.3))},
            # Original: trial.suggest_loguniform('p3', 1.1, 1.1)
            {"name": "p3", "type": "continuous", "domain": (np.log(1.1), np.log(1.1))},
            # Original: trial.suggest_int('p4', -100, 8)
            {"name": "p4", "type": "discrete", "domain": (0, (8 - (-100)) // 1)},
            # Original: trial.suggest_int('p5', -20, -20)
            {"name": "p5", "type": "discrete", "domain": (0, (-20 - (-20)) // 1)},
            # Original: trial.suggest_int('p6', 1, 8, log=True)
            {"name": "p6", "type": "continuous", "domain": (np.log(1 - 0.5), np.log(8 + 0.5))},
            # Original: trial.suggest_discrete_uniform('p7', 10, 20, 2)
            {"name": "p7", "type": "discrete", "domain": (0, (20 - 10) // 2)},
            # Original: trial.suggest_discrete_uniform('p8', 0.1, 1.0, 0.1)
            {"name": "p8", "type": "discrete", "domain": (0, int((1.0 - 0.1) // 0.1))},
            # Original: trial.suggest_discrete_uniform('p9', 2.2, 2.2, 0.5)
            {"name": "p9", "type": "discrete", "domain": (0, int((2.2 - 2.2) // 0.5))},
        ]

        _assert_2domains_are_equal(domain, expected_domain)


def test_gpyopt_kwargs() -> None:
    warnings.filterwarnings("ignore", category=optuna.exceptions.ExperimentalWarning)
    sampler = optuna.integration.GpyoptSampler(gpyopt_kwargs={"acquisition_type": "LCB"})
    study = optuna.create_study(sampler=sampler)
    ...
    with patch("GPyOpt.methods.BayesianOptimization") as mock_object:
        ...
        domain = mock_object.mock_calls[0][2]["domain"]
        expected_domain = [{"name": "x", "type": "discrete", "domain": (0, 20)}]

        _assert_2domains_are_equal(domain, expected_domain)

        assert mock_object.mock_calls[0][2]["acquisition_type"] == "LCB"


def test_gpyopt_kwargs_dimensions() -> None:
    warnings.filterwarnings("ignore", category=optuna.exceptions.ExperimentalWarning)
    # User specified `domain` argument will be ignored in `GpyoptSampler`.
    sampler = optuna.integration.GpyoptSampler(gpyopt_kwargs={"domain": []})
    study = optuna.create_study(sampler=sampler)
...
def test_is_compatible() -> None:
    ...
    controller = optuna.integration.gpyopt._GpyoptController(search_space, {})

    # Compatible.
    trial = _create_frozen_trial(
        {"p0": 5}, {"p0": distributions.UniformDistribution(low=0, high=10)}
    )
    assert controller._is_compatible(trial)
    ...
    assert not controller._is_compatible(trial)
    ...
    with pytest.raises(ValueError):
        controller._is_compatible(trial)
...
def test_sample_relative_n_startup_trials() -> None:
    warnings.filterwarnings("ignore", category=optuna.exceptions.ExperimentalWarning)
    warnings.filterwarnings("ignore", module="paramz", category=DeprecationWarning)
    warnings.filterwarnings("ignore", module="scipy", category=DeprecationWarning)
    ...
    # GpyoptSampler is used for Trial#2.
    with patch.object(
...
def test_get_trials() -> None:
    ...
    with patch("optuna.Study.get_trials", new=Mock(side_effect=lambda deepcopy: _create_trials())):
        sampler = optuna.integration.GpyoptSampler(consider_pruned_trials=False)
        study = optuna.create_study(sampler=sampler)
        trials = sampler._get_trials(study)
        assert len(trials) == 1

        sampler = optuna.integration.GpyoptSampler(consider_pruned_trials=True)
        study = optuna.create_study(sampler=sampler)
        trials = sampler._get_trials(study)
        assert len(trials) == 2
...
def _assert_2domains_are_equal(domain: List[Any], expected_domain: List[Any]) -> None:
    for d1, d2 in zip(domain, expected_domain):
        assert isinstance(d1, dict) and isinstance(d2, dict)
        assert d1["name"] == d2["name"]
        assert d1["type"] == d2["type"]
        if d1["type"] == "categorical":
            assert tuple(d1["domain"]) == tuple(d2["domain"])
        else:
            assert np.isclose(d1["domain"][0], d2["domain"][0])
            assert np.isclose(d1["domain"][1], d2["domain"][1])
