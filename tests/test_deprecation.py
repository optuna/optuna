...
@pytest.mark.parametrize("deprecated_version", ["1.1", 100, None])
@pytest.mark.parametrize("removed_version", ["1.1", 10, None])
def test_deprecation_raises_error_for_invalid_version(
    deprecated_version: Any, removed_version: Any
) -> None:
    with pytest.raises(ValueError):
        _deprecation.deprecated(deprecated_version, removed_version)


def test_deprecation_decorator() -> None:
    deprecated_version = "1.1.0"
    removed_version = "3.0.0"
    decorator_deprecation = _deprecation.deprecated(deprecated_version, removed_version)
    ...
    assert decorated_func.__doc__ == _deprecation._DEPRECATION_NOTE_TEMPLATE.format(
...
def test_deprecation_class_decorator() -> None:
    ...
    assert decorated_class.__doc__ == _deprecation._DEPRECATION_NOTE_TEMPLATE.format(
        d_ver=deprecated_version, r_ver=removed_version
    )
...
def test_deprecation_class_decorator_name() -> None:
    ...
    name = "foo"
    decorator_deprecation = _deprecation.deprecated("1.1.0", "3.0.0", name=name)
