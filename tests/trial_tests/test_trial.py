...
def test_check_distribution_suggest_uniform(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        assert len(warning_records) == 1, [
            warning.message for warning in warning_records
        ]
...
def test_suggest_uniform(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        assert (
            trial.suggest_uniform("x", 0.0, 3.0) == 1.0
        )  # Test suggesting the same param.
        assert (
            trial.suggest_uniform("y", 0.0, 3.0) == 2.0
...
def test_suggest_loguniform(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        assert (
            trial.suggest_loguniform("x", 0.1, 4.0) == 1.0
        )  # Test suggesting a param.
        assert (
            trial.suggest_loguniform("x", 0.1, 4.0) == 1.0
        )  # Test suggesting the same param.
        assert (
            trial.suggest_loguniform("y", 0.1, 4.0) == 2.0
        )  # Test suggesting a different param.
        assert trial.params == {"x": 1.0, "y": 2.0}
...
def test_suggest_low_equals_high(storage_mode: str) -> None:
    with (
        ...
        study = create_study(
            storage=storage, sampler=samplers.TPESampler(n_startup_trials=0)
        )
        ...
        assert (
            trial.suggest_loguniform("b", 1.0, 1.0) == 1.0
...
def test_suggest_discrete_uniform_range(
    storage_mode: str, range_config: dict[str, float]
) -> None:
    sampler = samplers.RandomSampler()
...
def test_suggest_int_log(storage_mode: str) -> None:
    ...
    with StorageSupplier(storage_mode) as storage:
        ...
        assert (
            trial.suggest_int("x", 1, 3, log=True) == 1
        )  # Test suggesting the same param.
        assert (
            trial.suggest_int("y", 1, 3, log=True) == 2
        )  # Test suggesting a different param.
        assert trial.params == {"x": 1, "y": 2}
