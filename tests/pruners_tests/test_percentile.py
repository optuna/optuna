...
def test_get_best_intermediate_result_over_steps(direction_expected):
    ...
    assert math.isnan(percentile._get_best_intermediate_result_over_steps(
        frozen_trial_nan, direction))
...
def test_get_percentile_intermediate_result_over_trials():
    ...
    def setup_study(trial_num, _intermediate_values):
        ...
        trial_ids = [_study.storage.create_new_trial_id(_study.study_id) for _ in range(trial_num)]

        for step, values in enumerate(_intermediate_values):
            # Study does not have any trials.
            with pytest.raises(ValueError):
                _all_trials = _study.storage.get_all_trials(_study.study_id)
                _direction = _study.storage.get_study_direction(_study.study_id)
            ...
            for i in range(trial_num):
                ...
                value = values[i]
                _study.storage.set_trial_intermediate_value(trial_id, step, value)

        # Set trial states complete because this method ignores incomplete trials.
        for trial_id in trial_ids:
            _study.storage.set_trial_state(trial_id, TrialState.COMPLETE)
    ...
    all_trials = study.storage.get_all_trials(study.study_id)
    direction = study.storage.get_study_direction(study.study_id)
