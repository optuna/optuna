...
def test_successive_halving_pruner_rung_check():
    ...
    for i in range(7):
        ...
        pruner.prune(study=study, trial=study._storage.get_trial(trial.trial_id), step=7)
...
def test_successive_halving_pruner_first_trial_is_not_pruned():
    ...
    for i in range(10):
        ...
        assert not pruner.prune(study=study,
                                trial=study._storage.get_trial(trial.trial_id), step=i)
...
def test_successive_halving_pruner_reduction_factor_parameter():
    ...
    assert not pruner.prune(study=study, trial=study._storage.get_trial(trial.trial_id), step=3)
    assert 'completed_rung_1' in trial.system_attrs
    assert 'completed_rung_2' not in trial.system_attrs
