...
# Tests in this file cover the behavior that every importance evaluator must satisfy, so they are
# parametrized over all the evaluators. The behavior that differs between the evaluators is covered
# by ``test_tree_based_evaluator.py`` (tree-based evaluators) and the dedicated tests under
# ``pedanova_tests`` and ``fanova_tests``.
...
parametrize_evaluator_cls = pytest.mark.parametrize("evaluator_cls", evaluators)
...
def test_get_param_importance_target_is_none_and_study_is_multi_obj(
    ...
    with pytest.raises(ValueError):
        get_param_importances(study, evaluator=evaluator_cls())
