...
def test_wfg_2d() -> None:
    for n in range(2, 30):
        ...
        np.random.shuffle(s)
        v = optuna.samplers._motpe_hypervolume.WFG().compute(s, r)
...
def test_wfg_duplicate_points() -> None:
    ...
    np.random.shuffle(s)
    v_with_duplicate_point = optuna.samplers._motpe_hypervolume.WFG().compute(s, r)
...
def test_invalid_input() -> None:
    ...
    with pytest.raises(ValueError):
        _ = optuna.samplers._motpe_hypervolume.WFG().compute(s, r)
