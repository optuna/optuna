...
def test_constrained_dominates() -> None:

    directions = [StudyDirection.MINIMIZE, StudyDirection.MAXIMIZE]

    def check_hasse_diagram(
        trials: List[FrozenTrial], inequalities: List[Tuple[int, int]]
    ) -> None:
        # Check that all inequalities transitively generated by the given list of
        # inequalities are satisfied, and that all other inequalities are not satisfied.

        dic = {t.number: t for t in trials}
        for t in trials:
            # Get all trials dominated by t
            dominated = {t.number}
            last_len = 0
            while len(dominated) > last_len:
                last_len = len(dominated)
                dominated |= {
                    t2.number
                    for t2 in trials
                    if any((i, t2.number) in inequalities for i in dominated)
                }
            dominated -= {t.number}

            for i in dominated:
                assert _constrained_dominates(t, dic[i], directions)
            for i in set(dic.keys()) - dominated:
                assert not _constrained_dominates(t, dic[i], directions)

    trials = [
        _create_frozen_trial(0, [-5, 5], [-float("inf"), -2]),
        _create_frozen_trial(1, [0, 5], [0, 0]),
        _create_frozen_trial(2, [-5, 0], [-1, -2]),
        _create_frozen_trial(3, [float("inf"), 5], [-1, -1]),
        _create_frozen_trial(4, [float("inf"), 5], [0, -1]),
        _create_frozen_trial(5, [float("inf"), -float("inf")], [0, 0]),
        _create_frozen_trial(6, [0, 0], [2, -2]),
        _create_frozen_trial(7, [-5, 0], [1, 2]),
        _create_frozen_trial(8, [-5, 5], [-2, 3]),
        _create_frozen_trial(9, [0, 5], [float("inf"), -float("inf")]),
        _create_frozen_trial(10, [-5, 5], [float("inf"), float("inf")]),
    ]
    ...
    with warnings.catch_warnings():
        ...
        for i in range(len(population_per_rank) - 1):
            for trial2 in population_per_rank[i + 1]:
                assert any(
                    _constrained_dominates(trial1, trial2, direction) for trial1 in population_per_rank[i]
            )
...
def test_fast_non_dominated_sort_no_constraints(values_dim: int) -> None:
    ...
    values = itertools.product([10, 20, 20, 30, float("inf"), float("inf"), -float("inf")], repeat=values_dim)
...
def test_fast_non_dominated_sort_constrained_feasible(values_dim, constraints_dim) -> None:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", optuna.exceptions.ExperimentalWarning)
        sampler = NSGAIISampler(constraints_func=lambda _: [0])

    values = itertools.product([10, 20, 20, 30, float("inf"), float("inf"), -float("inf")], repeat=values_dim)
    constraints = itertools.product([-float("inf"), -2, 0, 1, 2, 3, float("inf"), float("nan")], repeat=constraints_dim)
...
@pytest.mark.parametrize("values_and_constraints", 
    [[([10], None), ([20], None), ([20], [0]), ([20], [1]), ([30], [-1])],
    [([50, 30], None), ([30, 50], None), ([20, 20], [3, 3]), ([30, 10], [0, -1]), ([15, 15], [4, 4])],])
