...
def test_constrained_dominates() -> None:
    ...
    def check_hasse_diagram(trials: List[FrozenTrial], inequalities: List[Tuple[int, int]]):
        ...
        for t in trials:
            ...
            while len(dominated) > last_len:
                ...
                dominated |= {t2.number for t2 in trials if any((i, t2.number) in inequalities for i in dominated)}
            dominated -= {t.number}
    ...
    trials = [_create_frozen_trial(0, [-5, 5], [-float("inf"), -2]),
