...
class TestSampler(SamplerTestCase):
    ...
    def test_reseed_rng_change_sampling(
        self, sampler_class_with_seed: Callable[[int], BaseSampler]
    ) -> None:
        def objective(trial: Trial) -> float:
            a = trial.suggest_float("a", 1, 9)
            b = trial.suggest_float("b", 1, 9, log=True)
    ...
    def test_reproducible(
        self, sampler_class_with_seed: Callable[[int], BaseSampler], objective_func: Any
    ) -> None:
        def objective(trial: Trial) -> float:
            a = trial.suggest_float("a", 1, 9)
            b = trial.suggest_float("b", 1, 9, log=True)
    ...
    def test_reproducible_in_other_process(
        self, sampler_name: str, unset_seed_in_test: None
    ) -> None:
    ...
    def test_pickle_random_sampler(self, seed: int) -> None:
